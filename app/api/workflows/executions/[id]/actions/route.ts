...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    switch (action) {
      case 'pause': await WorkflowRuntimeService.pauseExecution(id); break;
      case 'resume': await WorkflowRuntimeService.resumeExecution(id); break;
      case 'cancel': await WorkflowRuntimeService.cancelExecution(id); break;
      case 'retry': await WorkflowRuntimeService.retryExecution(id, fromStep); break;
      default: return errorResponse('Invalid action. Use pause | resume | cancel | retry', 400);
    }
    const updated = await WorkflowRuntimeService.getExecutionStatus(id);
