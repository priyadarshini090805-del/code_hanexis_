...
export default function AnalyticsPage() {
  ...
  if (loading) return <div className="p-8 text-center text-[var(--hx-text-secondary)]">Loading analytics...</div>;
  ...
  const kpis = [
    { label: 'Total Leads', value: totalLeads, sub: 'all statuses', trend: '+', color: 'text-[var(--hx-text-secondary)]' },
    { label: 'Contacted', value: data?.leadStats?.CONTACTED || 0, sub: 'leads reached', trend: '+', color: 'text-[var(--hx-text-secondary)]' },
    { label: 'Conversion', value: `${conversionRate}%`, sub: 'leads converted', trend: '+', color: 'text-[var(--hx-text-secondary)]' },
    { label: 'Campaigns', value: data?.completedCampaigns || 0, sub: 'completed', trend: '+', color: 'text-[var(--hx-text-secondary)]' },
  ];
  ...
  return (
    <div className="min-h-screen bg-[var(--hx-bg)]">
      <div className="border-b border-[var(--hx-border)] px-6 py-4 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--hx-text)]">Analytics</h1>
          <p className="text-sm text-[var(--hx-text-secondary)] mt-0.5">Track your sales and outreach performance</p>
        </div>
        <div className="flex items-center gap-3">
          <input type="date" value={dateRange.from}
            onChange={(e) => setDateRange({ ...dateRange, from: e.target.value })}
            className="px-3 py-1.5 text-sm border border-[var(--hx-border)] rounded-lg focus:outline-none focus:ring-2 focus:ring-black"
          />
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        {error && <div className="p-4 bg-[var(--hx-surface-secondary)] border border-[var(--hx-border)] rounded-lg text-[var(--hx-text)] text-sm">{error}</div>}

        {/* KPI Cards */}
        <div className="grid grid-cols-4 gap-4">
          {kpis.map((kpi, i) => (
            <div key={i} className="border border-[var(--hx-border)] rounded-xl p-5">
              <p className="text-xs text-[var(--hx-text-secondary)] uppercase tracking-wider">{kpi.label}</p>
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="col-span-2 border border-[var(--hx-border)] rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-[var(--hx-text)]">Leads Over Time</h2>
              <div className="flex gap-4 text-xs">
                {[
                  ...
                  <span key={l.label} className="flex items-center gap-1">
                    ...
                    <span className="text-[var(--hx-text-secondary)]">{l.label}</span>
                  </span>
                ))}
              </div>
            </div>
          ...
          <div className="border border-[var(--hx-border)] rounded-xl p-5">
            <h2 className="font-semibold text-[var(--hx-text)] mb-4">Lead Funnel</h2>
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="border border-[var(--hx-border)] rounded-xl p-5">
            <h2 className="font-semibold text-[var(--hx-text)] mb-4">Lead Status Breakdown</h2>
            <BarChart data={leadStatusBars} />
          </div>

          <div className="border border-[var(--hx-border)] rounded-xl p-5">
            <h2 className="font-semibold text-[var(--hx-text)] mb-4">AI & Campaign Stats</h2>
            <div className="space-y-4">
              {[
                { label: 'Messages Generated', value: data?.totalMessagesGenerated || 0, color: 'bg-[var(--hx-surface-secondary)]0' },
                { label: 'Completed Campaigns', value: data?.completedCampaigns || 0, color: 'bg-[var(--hx-surface-secondary)]0' },
                { label: 'AI Tokens Used', value: latestMetrics.tokensUsed || 0, color: 'bg-[var(--hx-surface-secondary)]0' },
              ].map((s) => {
                ...
                return (
                  <div key={s.label}>
                    <div className="flex justify-between text-sm mb-1">
                      <span className="text-[var(--hx-text-secondary)]">{s.label}</span>
                      <span className="font-semibold text-[var(--hx-text)]">{s.value.toLocaleString()}</span>
                    </div>
              ...
              <div className="border-t border-gray-100 pt-4 mt-4">
                <h3 className="text-sm font-semibold text-[var(--hx-text)] mb-3">Messages Over Time</h3>
                <LineChart
                  data={data?.analytics || []}
        ...
        <div className="border border-[var(--hx-border)] rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100">
            <h2 className="font-semibold text-[var(--hx-text)]">Daily Metrics</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-[var(--hx-surface-secondary)]">
                <tr>
                  {['Date', 'New Leads', 'Contacted', 'Converted', 'Conv. Rate', 'Messages'].map((h) => (
                    <th key={h} className="px-4 py-2.5 text-left text-xs font-semibold text-[var(--hx-text-secondary)] uppercase tracking-wider">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {(data?.analytics || []).slice(0, 10).map((metric, i) => (
                  <tr key={i} className="border-t border-gray-50 hover:bg-[var(--hx-surface-secondary)]">
                    <td className="px-4 py-3 text-[var(--hx-text-secondary)]">
                      {new Date(metric.date).toLocaleDateString([], { month: 'short', day: 'numeric', year: 'numeric' })}
                    </td>
                    <td className="px-4 py-3 font-medium text-[var(--hx-text)]">{metric.newLeads}</td>
                    <td className="px-4 py-3 font-medium text-[var(--hx-text)]">{metric.contactedLeads}</td>
                    <td className="px-4 py-3 font-medium text-[var(--hx-text)]">{metric.convertedLeads}</td>
                    <td className="px-4 py-3 font-medium text-[var(--hx-text)]">{Math.round(metric.conversionRate)}%</td>
                    <td className="px-4 py-3 font-medium text-[var(--hx-text)]">{metric.messagesGenerated}</td>
                  </tr>
                ))}
